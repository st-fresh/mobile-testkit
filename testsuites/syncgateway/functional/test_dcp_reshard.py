...
def test_dcp_reshard_sync_gateway_goes_down(conf):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        ...
        futures[executor.submit(seth.add_docs, 1)] = "seth"

        log.info(">>> Adding Traun docs")  # ABC, NBC, CBS
        futures[executor.submit(traun.add_docs, 1, bulk=True)] = "traun"
    ...
    verify_changes(traun, expected_num_docs=1, expected_num_revisions=0, expected_docs=traun.cache)
    verify_changes(seth, expected_num_docs=1, expected_num_revisions=0, expected_docs=seth.cache)
...
def test_dcp_reshard_sync_gateway_comes_up(conf):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        ...
        log.info(">>> Adding Traun docs")  # ABC, NBC, CBS
        futures[executor.submit(traun.add_docs, 1)] = "traun"
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            if tag == "sg_accel_down":
                ...
                log.info(">>> Adding Seth docs")  # FOX
                futures[executor.submit(seth.add_docs, 1, bulk=True)] = "seth"
