...
test bulk get compression no compression
    test bulk get compression   ${SYNC_GATEWAY_CONFIGS}/sync_gateway_default_cc.json    ${1}

test bulk get compression no compression 1.1 user agent
    test bulk get compression   ${SYNC_GATEWAY_CONFIGS}/sync_gateway_default_cc.json    ${1}  user_agent=CouchbaseLite/1.1

test bulk get compression accept encoding gzip
    test bulk get compression   ${SYNC_GATEWAY_CONFIGS}/sync_gateway_default_cc.json    ${1}  accept_encoding=gzip

test bulk get compression accept encoding gzip 1.1 user agent
    test bulk get compression   ${SYNC_GATEWAY_CONFIGS}/sync_gateway_default_cc.json    ${1}  accept_encoding=gzip  user_agent=CouchbaseLite/1.1

test bulk get compression x accept part encoding gzip
    test bulk get compression   ${SYNC_GATEWAY_CONFIGS}/sync_gateway_default_cc.json    ${1}  x_accept_part_encoding=gzip

test bulk get compression x accept part encoding gzip 1.1 user agent
    test bulk get compression   ${SYNC_GATEWAY_CONFIGS}/sync_gateway_default_cc.json    ${1}  x_accept_part_encoding=gzip  user_agent=CouchbaseLite/1.1

test bulk get compression accept encoding gzip x accept part encoding gzip
    test bulk get compression   ${SYNC_GATEWAY_CONFIGS}/sync_gateway_default_cc.json    ${1}  accept_encoding=gzip  x_accept_part_encoding=gzip

test bulk get compression accept encoding gzip x accept part encoding gzip 1.1 user agent
    test bulk get compression   ${SYNC_GATEWAY_CONFIGS}/sync_gateway_default_cc.json    ${1}  accept_encoding=gzip  x_accept_part_encoding=gzip  user_agent=CouchbaseLite/1.1

test bulk get compression no compression 1.2 user agent
    test bulk get compression   ${SYNC_GATEWAY_CONFIGS}/sync_gateway_default_cc.json    ${1}  user_agent=CouchbaseLite/1.2

test bulk get compression accept encoding gzip 1.2 user agent
    test bulk get compression   ${SYNC_GATEWAY_CONFIGS}/sync_gateway_default_cc.json    ${1}  accept_encoding=gzip  user_agent=CouchbaseLite/1.2

test bulk get compression x accept part encoding gzip 1.2 user agent
    test bulk get compression   ${SYNC_GATEWAY_CONFIGS}/sync_gateway_default_cc.json    ${1}  x_accept_part_encoding=gzip  user_agent=CouchbaseLite/1.2

test bulk get compression accept encoding gzip x accept part encoding gzip 1.2 user agent
    test bulk get compression   ${SYNC_GATEWAY_CONFIGS}/sync_gateway_default_cc.json    ${1}  accept_encoding=gzip  x_accept_part_encoding=gzip  user_agent=CouchbaseLite/1.2


# test_continuous (channel cache mode)
test continuous changes parametrized 1 user 5000 docs 1 revision
    test continuous changes parametrized    ${SYNC_GATEWAY_CONFIGS}/sync_gateway_default_functional_tests_cc.json  ${1}  ${1}  ${1}
...
test continuous changes sanity
    test_continuous_changes_sanity          ${SYNC_GATEWAY_CONFIGS}/sync_gateway_default_functional_tests_cc.json  ${1}  ${1}


# test_db_online_offline (channel cache mode)
test online default rest
    test online default rest                                                                ${SYNC_GATEWAY_CONFIGS}/bucket_online_offline/bucket_online_offline_default_cc.json             ${1}

test offline false config_rest
    test offline false config_rest                                                          ${SYNC_GATEWAY_CONFIGS}/bucket_online_offline/bucket_online_offline_offline_false_cc.json       ${1}

test online to offline check 503
    test online to offline check 503                                                        ${SYNC_GATEWAY_CONFIGS}/bucket_online_offline/bucket_online_offline_default_cc.json           ${1}

test online to offline changes feed controlled close continuous
    test online to offline changes feed controlled close continuous                         ${SYNC_GATEWAY_CONFIGS}/bucket_online_offline/bucket_online_offline_default_cc.json   ${1}

test online to offline continous changes feed controlled close sanity mulitple users
    test online to offline continous changes feed controlled close sanity mulitple users    ${SYNC_GATEWAY_CONFIGS}/bucket_online_offline/bucket_online_offline_default_cc.json     ${1}     ${1}

test online to offline changes feed controlled close longpoll sanity
    test online to offline changes feed controlled close longpoll sanity                    ${SYNC_GATEWAY_CONFIGS}/bucket_online_offline/bucket_online_offline_default_cc.json     ${1}

test online to offline longpoll changes feed controlled close sanity mulitple users
    test online to offline longpoll changes feed controlled close sanity mulitple users     ${SYNC_GATEWAY_CONFIGS}/bucket_online_offline/bucket_online_offline_default_cc.json     ${1}     ${1}

test online to offline changes feed controlled close longpoll
    test online to offline changes feed controlled close longpoll                           ${SYNC_GATEWAY_CONFIGS}/bucket_online_offline/bucket_online_offline_default_cc.json     ${1}

test offline true config bring online
    test offline true config bring online                                                   ${SYNC_GATEWAY_CONFIGS}/bucket_online_offline/bucket_online_offline_offline_true_cc.json    ${1}

test db offline tap loss sanity dcp
    test db offline tap loss sanity                                                         ${SYNC_GATEWAY_CONFIGS}/bucket_online_offline/bucket_online_offline_default_dcp_cc.json     ${1}

test db offline tap loss sanity
    test db offline tap loss sanity                                                         ${SYNC_GATEWAY_CONFIGS}/bucket_online_offline/bucket_online_offline_default_cc.json     ${1}

test db delayed online
    test db delayed online                                                                  ${SYNC_GATEWAY_CONFIGS}/bucket_online_offline/bucket_online_offline_default_cc.json     ${1}

test multiple dbs unique buckets lose tap
    test multiple dbs unique buckets lose tap                                               ${SYNC_GATEWAY_CONFIGS}/bucket_online_offline/bucket_online_offline_multiple_dbs_unique_buckets_cc.json     ${1}


# test_db_online_offline_resync (channel cache mode)
test bucket online offline resync sanity
    test bucket online offline resync sanity    ${1}    ${1}  ${1}

test bucket online offline resync with online
    test bucket online offline resync with online   ${1}    ${1}  ${1}

test bucket online offline resync with offline
    test bucket online offline resync with offline  ${1}    ${1}  ${1}

# test_db_online_offline_webhooks (channel cache mode)
test webhooks
    test webhooks                                   ${1}    ${1}    ${1}    ${1}

test db online offline webhooks offline
    test db online offline webhooks offline         ${1}    ${1}    ${1}    ${1}

test db online offline_webhooks offline two
    test db online offline_webhooks offline two     ${1}    ${1}    ${1}    ${1}

# test_longpoll (channel cache mode)
test longpoll changes parametrized
    test longpoll changes parametrized      ${SYNC_GATEWAY_CONFIGS}/sync_gateway_default_functional_tests_cc.json   ${1}  ${1}

test longpoll changes parametrized
    test longpoll changes parametrized      ${SYNC_GATEWAY_CONFIGS}/sync_gateway_default_functional_tests_cc.json   ${1}   ${1}

test longpoll changes sanity
    test longpoll changes sanity            ${SYNC_GATEWAY_CONFIGS}/sync_gateway_default_functional_tests_cc.json   ${1}   ${1}


# test_multiple_dbs (channel cache mode)
test multiple db unique data bucket unique index bucket
    test multiple db unique data bucket unique index bucket     ${SYNC_GATEWAY_CONFIGS}/multiple_dbs_unique_data_unique_index_cc.json   ${1}   ${1}

test multiple db single data bucket single index bucket
    test multiple db single data bucket single index bucket     ${SYNC_GATEWAY_CONFIGS}/multiple_dbs_shared_data_shared_index_cc.json   ${1}   ${1}
...
test mulitple users mulitiple channels mulitple revisions
    test mulitple users mulitiple channels mulitple revisions   ${SYNC_GATEWAY_CONFIGS}/sync_gateway_default_functional_tests_cc.json   ${1}   ${1}    ${1}   ${1}


# test overloaded channel cache
test overloaded channel cache one
    test overloaded channel cache   ${SYNC_GATEWAY_CONFIGS}/sync_gateway_channel_cache_cc.json  ${1}  *    True   ${1}

test overloaded channel cache two
    test overloaded channel cache   ${SYNC_GATEWAY_CONFIGS}/sync_gateway_channel_cache_cc.json  ${1}  *    True   ${1}

test overloaded channel cache three
    test overloaded channel cache   ${SYNC_GATEWAY_CONFIGS}/sync_gateway_channel_cache_cc.json  ${1}  ABC  False  ${1}

test overloaded channel cache four
    test overloaded channel cache   ${SYNC_GATEWAY_CONFIGS}/sync_gateway_channel_cache_cc.json  ${1}  ABC  True   ${1}
...
test seq
    test seq        ${SYNC_GATEWAY_CONFIGS}/sync_gateway_default_functional_tests_cc.json   ${1}   ${1}  ${1}


# test test_single_user_single_channel_doc_updates (channel cache mode)
test single user single channel doc updates
    test single user single channel doc updates     ${SYNC_GATEWAY_CONFIGS}/sync_gateway_default_functional_tests_cc.json   ${1}  ${1}


# test_sync (channel cache mode)
test issue 1524
     test issue 1524            ${SYNC_GATEWAY_CONFIGS}/custom_sync/grant_access_one_cc.json   ${1}
